...
int main() {
	...
	printf("TESTS: Hand count should not increase, deck count should decrease by one, and num actions should increase by 1\n");

	// copy the game state to a test case
	memcpy(&testG, &G, sizeof(struct gameState));
	cardReturned = caseGreathall(thisPlayer, &testG, handpos);
